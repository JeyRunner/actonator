...
struct MotorControllerCanProtocol: public ProtocolDef<COMC, 21, 0, 0, 3> {
  ...
  struct ObjectTree: public Node {
		struct Settings: Node {
			...
			struct Motor_parameters: Node {
				ValueNodeWritable<TYPE_INT16, 1, COMC> encoderElectricalOffset;
				ValueNodeWritable<TYPE_UINT8, 2, COMC> electricalRotationsPerRevolution;
				ValueNodeWritableEnum<MOTOR_CONFIG_DIRECTION, 3, COMC> motorDirection;
				ValueNodeWritableEnum<BOOL, 4, COMC> motorParametersValid;
			} motor_parameters; 
		} settings; 
		struct Debug: Node {
			ValueNodeWritable<TYPE_UINT16, 5, COMC> clientRxBufferNumPackages;
			ValueNodeWritable<TYPE_UINT16, 6, COMC> clientTxBufferNumPackages;
		} debug; 
		struct Motor: Node {
			ValueNodeWritableEnum<MOTOR_OPERATION_MODE, 7, COMC> operationMode;
			...
			struct Current_values: Node {
				ValueNodeWritable<TYPE_F32, 10, COMC> position;
				ValueNodeWritable<TYPE_F32, 11, COMC> velocity;
				ValueNodeWritable<TYPE_F32, 12, COMC> velocityNonFiltered;
				ValueNodeWritable<TYPE_F32, 13, COMC> torque;
			} current_values; 
			...
			ValueNodeWritable<TYPE_F32, 18, COMC> temperature;
			struct Test_pd_mode: Node {
				ValueNodeReadWriteableEnum<BOOL, 19, COMC> active;
			} test_pd_mode; 
		} motor; 
		struct CliTool: Node {
			ValueNodeWritable<TYPE_UINT8, 20, COMC> version;
		} cliTool; 
  ...
  struct RemoteCalls {
		RemoteCallCallable<0, StartMotorInitSequenceArgDataCallable, StartMotorInitSequenceReturnDataCallable, VOID_ENUM, COMC> StartMotorInitSequence;
		RemoteCallCallable<1, SetMotorParametersArgDataCallable, VoidRemoteCallDataWritable, VOID_ENUM, COMC> SetMotorParameters;
		RemoteCallCallable<2, ChangeMotorOperationModeArgDataCallable, VoidRemoteCallDataWritable, REQUEST_MOTOR_OPERATION_MODE_ERROR, COMC> ChangeMotorOperationMode;
  ...
  RemoteCallCallerAbstract *rcCallerTable[0] = {

  };

  /// Remote calls that are callable on this device. The caller will be another device.
  /// the index is the call id belonging to the referenced remote call
  RemoteCallCallableAbstract *rcCallableTable[3] = {
		valueRemoteCallCallableAsAbstract(remoteCalls.StartMotorInitSequence),
		valueRemoteCallCallableAsAbstract(remoteCalls.SetMotorParameters),
		valueRemoteCallCallableAsAbstract(remoteCalls.ChangeMotorOperationMode),

  };


  explicit MotorControllerCanProtocol(COMC *sotCanCommunication)
  : ProtocolDef<COMC, 21, 0, 0, 3>(sotCanCommunication) {
